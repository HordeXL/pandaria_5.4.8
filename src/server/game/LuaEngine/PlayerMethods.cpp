...
namespace LuaPlayer
{
    ...
    int UnbindInstance(lua_State* L, Player* player)
    {
        ...
        if (difficulty <= 0)
            player->UnbindInstance(map, Difficulty(difficulty), false);
        return 0;
    }
    ...
    int SummonPet(lua_State* L, Player* player)
    {
        ...
        player->SummonPet(entry, x, y, z, o, despwtime);
        return 0;
    }
    ...
    int GiveXP(lua_State* L, Player* player)
    {
        ...
        if (player->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_NO_XP_GAIN))
            return 0;
        if (victim && victim->GetTypeId() == TYPEID_UNIT && victim->ToCreature()->GetLootRecipient() == nullptr)
            return 0;
        ...
        uint32 curXP = player->GetUInt32Value(PLAYER_FIELD_XP);
        ...
        player->SetUInt32Value(PLAYER_FIELD_XP, newXP);
        return 0;
    }
    ...
    {
        sEluna->Push(L, player->GetCurrency(CurrencyTypes::CURRENCY_TYPE_HONOR_POINTS, true));
        return 1;
    }
