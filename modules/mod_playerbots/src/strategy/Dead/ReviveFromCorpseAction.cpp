...
bool ReviveFromCorpseAction::Execute(Event event)
{
    ...
    TC_LOG_DEBUG("playerbots", "Bot %s %s:%u <%s> revives at body", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(),
...
WorldSafeLocsEntry const* SpiritHealerAction::GetGrave(bool startZone)
{
    ...
    ClosestGrave = sObjectMgr->GetClosestGraveYard(WorldLocation(bot->GetMapId(), bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ(), bot->GetOrientation()), bot->GetTeamId(), bot);
    ...
    if (botAI->HasStrategy("follow", BOT_STATE_NON_COMBAT) && botAI->GetGroupMaster() && botAI->GetGroupMaster() != bot)
    {
        ...
        {
            ClosestGrave = sObjectMgr->GetClosestGraveYard(WorldLocation(master->GetMapId(), master->GetPositionX(), master->GetPositionY(), master->GetPositionZ(), master->GetOrientation()), bot->GetTeamId(), bot);
...
bool SpiritHealerAction::Execute(Event event)
{
    ...
    if (bot->GetDistance2d(ClosestGrave->x, ClosestGrave->y) < sPlayerbotAIConfig->sightDistance)
    {
        ...
        for (GuidVector::iterator i = npcs.begin(); i != npcs.end(); i++)
        {
            ...
            {
                TC_LOG_DEBUG("playerbots", "Bot %s %s:%u <%s> revives at spirit healer", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(), bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName().c_str());
...
{
    return bot->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_GHOST);
}
