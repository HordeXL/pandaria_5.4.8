...
bool ReleaseSpiritAction::Execute(Event event)
{
    ...
    if (bot->GetCorpse() && bot->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_GHOST))
    {
    ...
    TC_LOG_INFO("playerbots", "Bot %s %s:%u <%s> released", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(),
...
bool AutoReleaseSpiritAction::Execute(Event event)
{
    ...
    TC_LOG_DEBUG("playerbots", "Bot %s %s:%u <%s> auto released", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(),
              bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName().c_str());
    ...
    TC_LOG_DEBUG("playerbots", "Bot %s %s:%u <%s> releases spirit", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(),
              bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName().c_str());
...
bool AutoReleaseSpiritAction::isUseful()
{
    ...
    if (bot->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_GHOST))
        return false;
...
{
    TC_LOG_DEBUG("playerbots", "Bot %s %s:%u <%s> repops at graveyard", std::to_string(GUID_LOPART(bot->GetGUID())).c_str(),
              bot->GetTeamId() == TEAM_ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName().c_str());
    ...
    auto ClosestGraveyard = sObjectMgr->GetClosestGraveYard(WorldLocation(bot->GetMapId(), bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ(), bot->GetOrientation()), bot->GetTeamId(), bot);
    if (!ClosestGraveyard)
        return false;
