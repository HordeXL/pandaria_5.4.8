...
void MovementAction::UpdateMovementState()
{
    ...
    ZLiquidStatus botInLiquidState = bot->GetMap()->getLiquidStatus(bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ(), MAP_ALL_LIQUIDS, &liquid_status);
    ...
    bool onGround = bot->GetPositionZ() < bot->GetMap()->GetWaterOrGroundLevel(bot->GetPhaseMask(), bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ()) + 1.0f;
...
bool MovementAction::ChaseTo(WorldObject* obj, float distance, float angle)
{
    ...
    if (Vehicle* vehicle = bot->GetVehicle())
    {
        ...
        if (!seat || !(seat->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL))
            return false;
...
bool MovementAction::ReachCombatTo(Unit* target, float distance)
{
    ...
    // ShortenPathUntilDist not available in this core - use full path
    ...
    bool exact_waypoint, MovementPriority priority, bool lessDelay, bool backwards)
{
    ...
    if (Vehicle* vehicle = bot->GetVehicle())
    {
        ...
        if (!vehicleBase || !seat || !(seat->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL))  // is passenger and cant move anyway
            return false;
...
bool MoveRandomAction::Execute(Event event)
{
    ...
    while (--attempts)
    {
        ...
        float dz = z;
        if (false) // CheckCollisionAndGetValidCoords not available in this core
            continue;
            continue;

        if (map->IsInWater(dx, dy, dz))
            continue;
...
bool MovementAction::MoveNear(WorldObject* target, float distance, MovementPriority priority)
{
    ...
    distance += (target->GetTypeId() == TYPEID_UNIT || target->GetTypeId() == TYPEID_PLAYER) ? ((Unit*)target)->GetCombatReach() : 0.0f;
...
bool MovementAction::IsMovingAllowed()
{
    ...
    if (bot->isFrozen() || bot->IsPolymorphed() || (bot->isDead() && !bot->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_GHOST)) ||
        bot->IsBeingTeleported() || bot->HasAuraWithMechanic(1 << MECHANIC_ROOT) || bot->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION) ||
        bot->HasAuraWithMechanic(1 << MECHANIC_DISORIENTED) || bot->IsCharmed() || bot->HasAuraWithMechanic(1 << MECHANIC_STUN) ||
    ...
    int maxSearchCount, bool normal_only, float step)
{
    ...
    float tempZ = bot->GetMap()->GetHeight(bot->GetPhaseMask(), x, y, z);
    PathGenerator gen(bot);
    gen.CalculatePath(x, y, tempZ);
    Movement::PointsArray result = gen.GetPath();
    auto pathLength = [](Movement::PointsArray const& path) {
        float len = 0.0f;
        for (size_t i = 1; i < path.size(); ++i)
            len += (path[i - 1] - path[i]).length();
        return len;
    };
    float min_length = pathLength(gen.GetPath());
    ...
    {
        tempZ = bot->GetMap()->GetHeight(bot->GetPhaseMask(), x, y, z + delta);
        ...
        PathGenerator gen(bot);
        gen.CalculatePath(x, y, tempZ);
        if ((gen.GetPathType() & typeOk) && pathLength(gen.GetPath()) < min_length)
        {
            found = true;
            min_length = pathLength(gen.GetPath());
            result = gen.GetPath();
            modified_z = tempZ;
        }
    }
...
bool MovementAction::Move(float angle, float distance)
{
    ...
    float z = bot->GetMap()->GetWaterOrGroundLevel(bot->GetPhaseMask(), x, y, bot->GetPositionZ());
...
Position MovementAction::BestPositionForMeleeToFlee(Position pos, float radius)
{
    ...
    for (CheckAngle& checkAngle : possibleAngles)
    {
        ...
        Position fleePos; fleePos.Relocate(dx, dy, dz);
...
{
    if (bot->isFrozen() || bot->IsPolymorphed() || (bot->isDead() && !bot->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_GHOST)) ||
        bot->IsBeingTeleported() || bot->HasAuraWithMechanic(1 << MECHANIC_DISORIENTED) || bot->IsCharmed() ||
        bot->HasAuraWithMechanic(1 << MECHANIC_STUN) || bot->IsInFlight() ||
...
Position CombatFormationMoveAction::AverageGroupPos(float dis, bool ranged, bool self)
{
    ...
    averageY /= cnt;
    averageZ /= cnt;
    Position result; result.Relocate(averageX, averageY, averageZ); return result;
}
...
bool TankFaceAction::Execute(Event event)
{
    ...
    // if dist < NOMINAL_MELEE_RANGE / 2, target will move backward
    float dist = std::max(bot->GetExactDist(target), NOMINAL_MELEE_RANGE / 2) - bot->GetCombatReach() - target->GetCombatReach();
    std::vector<Position> availablePos;
    float x, y, z;
    target->GetNearPoint(bot, x, y, z, 0.0f, dist, goodAngle1);
    if (true) // CheckCollisionAndGetValidCoords not available in this core
        // position validity check skipped
    {
        ...
        if (CheckLastFlee(angle, infoList))
        {
            Position p; p.Relocate(x, y, z); availablePos.push_back(p);
        }
    }
