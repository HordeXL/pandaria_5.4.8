...
Unit* EnemyPlayerValue::Calculate()
{
    ...
    if (Vehicle* vehicle = bot->GetVehicle())
    {
        ...
        if (!seat || !(seat->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL))  // not in control of vehicle so cant attack anyone
            return nullptr;
    ...
    while (pReference)
    {
        ...
        {
            if (pTarget != pVictim && pTarget->ToPlayer() && pTarget->CanSeeOrDetect(bot) &&
                bot->IsWithinDist(pTarget, MAX_VISIBILITY_DISTANCE))
            {
        ...
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Unit* pMember = itr->GetSource())
            {
                ...
                if (Unit* pAttacker = pMember->getAttackerForHelper())
                    if (pAttacker->ToPlayer() && bot->IsWithinDist(pAttacker, maxAggroDistance * 2.0f) &&
