...
{
    m_mapId = pos.m_mapId;
    Relocate(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation());
}

void WorldPosition::set(const WorldObject* pos)
{
    m_mapId = pos->GetMapId();
    Relocate(pos->GetPositionX(), pos->GetPositionY(), pos->GetPositionZ(), pos->GetOrientation());
}
...
{
    return getMap() ? getMap()->IsInWater(GetPositionX(), GetPositionY(), GetPositionZ())
        : false;
};

bool WorldPosition::isUnderWater()
{
    return getMap() ? getMap()->IsUnderWater(GetPositionX(), GetPositionY(), GetPositionZ())
        : false;
};
...
uint16 WorldPosition::getAreaId() { return static_cast<uint16>(sMapMgr->GetAreaId(getMapId(), getX(), getY(), getZ())); }
...
GuidPosition::GuidPosition(CreatureData const& creData)
    : ObjectGuid(MAKE_NEW_GUID(creData.id, 0, HIGHGUID_UNIT)),
...
GuidPosition::GuidPosition(GameObjectData const& goData)
    : ObjectGuid(MAKE_NEW_GUID(goData.id, 0, HIGHGUID_GAMEOBJECT)),
...
{
    return IS_CREATURE_GUID(GetRawValue()) ? sObjectMgr->GetCreatureTemplate(GUID_ENPART(GetRawValue())) : nullptr;
}

GameObjectTemplate const* GuidPosition::GetGameObjectTemplate()
{
    return IS_GAMEOBJECT_GUID(GetRawValue()) ? sObjectMgr->GetGameObjectTemplate(GUID_ENPART(GetRawValue())) : nullptr;
}
...
WorldObject* GuidPosition::GetWorldObject()
{
    ...
    switch (GUID_HIPART(GetRawValue()))
    {
...
{
    return IS_CREATURE_GUID(GetRawValue()) && GetCreatureTemplate()->npcflag & flag;
}
...
Unit* GuidPosition::GetUnit()
{
    ...
    if (IS_PLAYER_GUID(GetRawValue()))
        return ObjectAccessor::FindPlayer(*this);

    if (IS_PET_GUID(GetRawValue()))
        return GetCreature();
...
bool GuidPosition::isDead()
{
    ...
    if (IS_CRE_OR_VEH_OR_PET_GUID(GetRawValue()) && GetUnit() && GetUnit()->IsInWorld() && GetUnit()->IsAlive())
        return false;

    if (IS_GAMEOBJECT_GUID(GetRawValue()) && GetGameObject() && GetGameObject()->IsInWorld())
        return false;
