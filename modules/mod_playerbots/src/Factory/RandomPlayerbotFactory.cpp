...
Player* RandomPlayerbotFactory::CreateRandomBot(WorldSession* session, Classes cls, std::unordered_map<Gender, std::vector<std::string>>& nameCache)
{
    ...
    // No CharSections.dbc in this codebase - use random default appearance values
    uint8 skinColor = static_cast<uint8>(std::rand() % 8);
    uint8 faceType = static_cast<uint8>(std::rand() % 5);
    uint8 faceColor = static_cast<uint8>(std::rand() % 8);
    uint8 hairType = static_cast<uint8>(std::rand() % 5);
    uint8 hairColor = static_cast<uint8>(std::rand() % 8);

    std::pair<uint8, uint8> face = std::pair<uint8, uint8>(faceType, faceColor);
    std::pair<uint8, uint8> hair = std::pair<uint8, uint8>(hairType, hairColor);

    bool excludeCheck = (race == RACE_TAUREN) || (race == RACE_DRAENEI) ||
        (gender == GENDER_FEMALE && race != RACE_NIGHTELF && race != RACE_UNDEAD_PLAYER);
    uint8 facialHair = excludeCheck ? 0 : static_cast<uint8>(std::rand() % 4);
    ...
    if (!player->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_PLAYER), characterInfo.get()))
    {
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    for (uint32 accountNumber = 0; accountNumber < sPlayerbotAIConfig->randomBotAccountCount; ++accountNumber)
    {
        ...
        for (uint8 cls = CLASS_WARRIOR; cls < MAX_CLASSES - character_count; ++cls)
        {
            ...
            bool const isClassDeathKnight = cls == CLASS_DEATH_KNIGHT;
            if (isClassDeathKnight && sWorld->getIntConfig(WorldIntConfigs::CONFIG_EXPANSION) < EXPANSION_WRATH_OF_THE_LICH_KING)
            {
                continue;
            }
            bool const isClassMonk = cls == CLASS_MONK;
            if (isClassMonk && sWorld->getIntConfig(WorldIntConfigs::CONFIG_EXPANSION) < EXPANSION_MISTS_OF_PANDARIA)
            {
                continue;
            }
