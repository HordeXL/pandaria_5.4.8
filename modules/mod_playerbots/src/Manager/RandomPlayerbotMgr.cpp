...
bool RandomPlayerbotMgr::ProcessBot(uint32 bot)
{
    ...
    ObjectGuid botGUID = ObjectGuid(MAKE_NEW_GUID(bot, 0, HIGHGUID_PLAYER));
...
void RandomPlayerbotMgr::RandomizeFirst(Player* bot)
{
    ...
    uint32 chance = std::min<uint32>(std::max<uint32>(sPlayerbotAIConfig->randomBotMaxLevelChance * 100, 0), 100);
    ...
    uint32 botLowGuid = GUID_LOPART(bot->GetGUID());

    PreparedStatement* stmt = PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_UPD_RANDOM_BOTS);
    stmt->setUInt32(0, randomTime);
    stmt->setString(1, "bot_delete");
    stmt->setUInt32(2, botLowGuid);
    PlayerbotsDatabase.Execute(stmt);
...
void RandomPlayerbotMgr::Randomize(Player* bot)
{
    ...
    // -- safe
    ScheduleRandomize(GUID_LOPART(bot->GetGUID()), 62208000);
}
...
bool RandomPlayerbotMgr::IsRandomBot(Player* bot)
{
    ...
    if (bot)
    {
        return IsRandomBot(GUID_LOPART(bot->GetGUID()));
    }
...
{
    ObjectGuid guid = ObjectGuid(MAKE_NEW_GUID(bot, 0, HIGHGUID_PLAYER));
...
void RandomPlayerbotMgr::GetBots()
{
    ...
    PreparedStatement* stmt = PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_RANDOM_BOTS_BY_OWNER_AND_EVENT);
    stmt->setUInt32(0, 0);
...
uint32 RandomPlayerbotMgr::GetEventValue(uint32 bot, std::string const event)
{
    ...
    {
        PreparedStatement* stmt = PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_RANDOM_BOTS_BY_OWNER_AND_BOT);
        stmt->setUInt32(0, 0);
...
{
    return GetValue(GUID_LOPART(bot->GetGUID()), type);
}
...
{
    SetValue(GUID_LOPART(bot->GetGUID()), type, value, data);
}
...
void RandomPlayerbotMgr::OnBotLoginInternal(Player* const bot)
{
    ...
    {
        // Allow player to get fully logged - execute immediately (no AddDelayedEvent in this core)
        {
    ...
    for (uint8 claz = CLASS_WARRIOR; claz <= CLASS_DRUID; claz++)
    {
        ...
        if (results)
        {
            do
            {
                ...
                ObjectGuid guid = ObjectGuid(MAKE_NEW_GUID(fields[0].GetUInt32(), 0, HIGHGUID_PLAYER));
