...
#include "SpellInfo.h"
#include "Transport.h"
// TradeData is defined in Player.h
#include "Unit.h"
#include "Vehicle.h"
...
bool PlayerbotAI::IsInVehicle(bool canControl, bool canCast, bool canAttack, bool canTurn, bool fixed)
{
    ...
    if (canControl)
        return (seat->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL) != 0;
...
{
    // GetUnit by CreatureData is not supported in this core - return nullptr
    return nullptr;
}
...
bool PlayerbotAI::Whisper(const std::string& msg, const std::string& receiverName)
{
    ...
    if (bot->GetTeamId() == TeamId::TEAM_ALLIANCE)
    {
        bot->Whisper(msg, LANG_COMMON, receiver->GetGUID());
    }
    else
    {
        bot->Whisper(msg, LANG_ORCISH, receiver->GetGUID());
    }
...
bool PlayerbotAI::IsSafe(WorldObject* obj)
{
    return obj && obj->GetMapId() == bot->GetMapId() && obj->GetInstanceId() == bot->GetInstanceId() &&
        (!obj->ToPlayer() || !((Player*)obj)->IsBeingTeleported());
}
